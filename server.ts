...
if (quotesCount.count === 0) {
  try {
    const seedData = JSON.parse(fs.readFileSync(path.resolve('quotes_seed.json'), 'utf-8'));
    const insertQuote = db.prepare('INSERT INTO quotes (id, person_name, text) VALUES (?, ?, ?)');
...
app.get('/api/drive', async (req, res) => {
  try {
    const apiKey = process.env.GOOGLE_API_KEY || 'AIzaSyB0OJVgWzssF45BsO1lsc1fWPe-Pm6-aeA';
    if (!apiKey) {
    ...
    const filteredFiles = (data.files || []).filter((f: any) => allowedMimeTypes.includes(f.mimeType));
...
app.post('/api/memos', (req, res) => {
  ...
  const id = uuidv4();
  db.prepare('INSERT INTO memos (id, title, body) VALUES (?, ?, ?)').run(id, title, body);
...
app.get('/api/likes', (req, res) => {
  ...
  const likes = db.prepare('SELECT content_id, COUNT(*) as count FROM likes GROUP BY content_id').all();
  res.json(likes);
});
...
app.post('/api/likes/:contentId', (req, res) => {
  ...
  // Toggle like
  const existing = db.prepare('SELECT * FROM likes WHERE content_id = ? AND user_id = ?').get(contentId, userId);
  
  if (existing) {
    db.prepare('DELETE FROM likes WHERE content_id = ? AND user_id = ?').run(contentId, userId);
    res.json({ liked: false });
  } else {
    db.prepare('INSERT INTO likes (content_id, user_id) VALUES (?, ?)').run(contentId, userId);
    res.json({ liked: true });
  }
});
...
app.get('/api/comments/:contentId', (req, res) => {
  const { contentId } = req.params;
  const comments = db.prepare('SELECT * FROM comments WHERE content_id = ? ORDER BY created_at ASC').all(contentId);
  res.json(comments);
});
...
app.post('/api/comments/:contentId', (req, res) => {
  ...
  if (!nickname || !text) return res.status(400).json({ error: 'Nickname and text are required' });
  
  const id = uuidv4();
  db.prepare('INSERT INTO comments (id, content_id, nickname, text) VALUES (?, ?, ?, ?)').run(id, contentId, nickname, text);
